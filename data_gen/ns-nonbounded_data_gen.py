import os
import numpy as np
import datasets

"""Hugging Face Datasets loader for non-bounded Navier-Stokes samples.

The training code consumes a two-channel `UA` tensor containing the selected
solution and coefficient fields.
"""

_CITATION = ""
_DESCRIPTION = "A dataset of .npy files containing U and A matrices."
_HOMEPAGE = ""

class MyMatrixDataset(datasets.GeneratorBasedBuilder):
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(name="default", version=datasets.Version("1.0.0"))
    ]

    def _info(self):
        return datasets.DatasetInfo(
            description=_DESCRIPTION,
            features=datasets.Features({
                "UA": datasets.Array3D(dtype="float32", shape=(2,128, 128)),
            }),
            supervised_keys=None,
            homepage=_HOMEPAGE,
            citation=_CITATION,
        )

    def _split_generators(self, dl_manager):
        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN,
                gen_kwargs={"data_dir": self.config.data_dir},
            )
        ]

    def _generate_examples(self, data_dir):
        for fname in sorted(os.listdir(data_dir)):
            if fname.endswith(".npy"):
                path = os.path.join(data_dir, fname)
                try:
                    data = np.load(path, allow_pickle=True)
                    U = data[...,-1].astype(np.float32).copy()
                    A =  data[...,0].astype(np.float32).copy()
                    yield fname, {
                        "UA": np.stack([U,A], axis = 0)
                    }
                except Exception as e:
                    print(f"Failed to load {fname}: {e}")
                    continue
