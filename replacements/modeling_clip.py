...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: CLIPConfig):
        ...
        if config.use_time:
            self.time_emb_proj = nn.Linear(768,self.embed_dim)
        if config.use_cond:
            self.cond_emb_proj = nn.Linear(768,self.embed_dim)
    ...
    def forward(
        ...
        if t_emb is not None and t_emb[0] is not None:
            emb = self.time_emb_proj(t_emb[0])
            hidden_states = hidden_states + emb.unsqueeze(-2)
        if t_emb is not None and t_emb[1] is not None:
            cond_emb = self.cond_emb_proj(t_emb[-1])
            hidden_states = hidden_states + cond_emb.unsqueeze(-2)
        residual = hidden_states
...
class CLIPVisionModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPVisionConfig, use_2_class = False):
        super().__init__(config)
        if True:
